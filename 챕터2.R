...
# 실습: union(), setdiff() 그리고 intersect() 함수를 
...
library(RSADBE)
date("Severity_Counts")
str("Severity_Counts")


#RSADBE 패키지에서 제공되는 데이터 셋
...
# 실습 :데이터 프레임의 칼럼명 참조하기기
df$x

# 실습 : 데이터프레임의 자료구조, 열 수 ,행 수, 칼럼명 참조하기
str(df)
ncol(df)
nrow(df)
names(df)
...
x1 <- subset(df,x>=3)  ## subset : 부분집합으로 만듦
x1
...
# 실습 : galton 데이터셋 가져오기, 중요X
##install.packages("UsingR")
library(UsingR)
data(galton)
data

# 실습 : key를 생락한 list 생성하기

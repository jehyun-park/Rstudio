...
h1 <- select(hflights_df, Year, Month, DepTime, ArrTime)
h1
h2 <- arrange(h1, desc(ArrTime))
...
da1 <- data.frame(x = 1:5, y = rnorm(5))
da2 <- data.frame(z = 1:5, y = qnorm(5))

dah <- bind_cols(da1,da2)
dah

dav <- bind_rows(da1,da2)
dav
...
data <- read.csv("c:/2022Rstudio/Rstudio//Part-II/data.csv")
data
library(reshape2)
